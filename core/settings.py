...
class AppConfig:
    ...
    def from_env(cls, env_path: str = ".env") -> "AppConfig":
        ...
        return cls(
            ...
            edge_tts_expressiveness=max(0, min(100, int(os.getenv("EDGE_TTS_EXPRESSIVENESS", "0")))),
            memory_store_path=os.getenv("MEMORY_STORE_PATH", os.path.join("data", "user_memory.json")),
            tts_chunk_chars=int(os.getenv("TTS_CHUNK_CHARS", "34")),
            tts_first_chunk_delay=float(os.getenv("TTS_FIRST_CHUNK_DELAY", "0.00")),
            tts_queue_timeout=float(os.getenv("TTS_QUEUE_TIMEOUT", "0.02")),
            tts_frames_per_buffer=int(os.getenv("TTS_FRAMES_PER_BUFFER", "512")),
            tts_playout_chunk_size=int(os.getenv("TTS_PLAYOUT_CHUNK_SIZE", "1024")),
            tts_min_sentence_length=int(os.getenv("TTS_MIN_SENTENCE_LENGTH", "10")),
            tts_min_first_fragment_length=int(os.getenv("TTS_MIN_FIRST_FRAGMENT_LENGTH", "14")),
