...
class ToolRegistrySystemControlTest(unittest.TestCase):
    def test_system_control_registered(self) -> None:
        ...
        definition = registry.get("system_control")

        self.assertIsNotNone(definition)
        assert definition is not None
        self.assertFalse(definition.safe_to_parallelize)

        valid, reason = registry.validate_args("system_control", {"action": "mute", "params": {}})
        self.assertTrue(valid, reason)

        invalid, _ = registry.validate_args("system_control", {"params": {}})
